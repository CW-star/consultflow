...
export default async function AnalyticsPage() {
  try {
    ...
    if (invError) return <div>Invoice error: {invError.message}</div>
    if (sessError) return <div>Session error: {sessError.message}</div>
        ...
        <div style={{ background: 'white', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '24px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '12px', height: '200px' }}>
            {monthlyData.length === 0 ? (
              <p style={{ color: '#9ca3af', fontSize: '14px' }}>No monthly data yet</p>
            ) : monthlyData.map(m => (
              <div key={m.month} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', height: '100%', justifyContent: 'flex-end' }}>
                <div style={{ width: '100%', display: 'flex', gap: '3px', alignItems: 'flex-end' }}>
                  <div style={{ flex: 1, background: '#dbeafe', borderRadius: '4px 4px 0 0', height: `${(m.billed / maxVal) * 160}px`, minHeight: '4px' }}/>
                  <div style={{ flex: 1, background: '#1e40af', borderRadius: '4px 4px 0 0', height: `${(m.collected / maxVal) * 160}px`, minHeight: '4px' }}/>
                </div>
                <p style={{ fontSize: '11px', color: '#9ca3af', margin: 0 }}>{m.month.slice(5)}</p>
              </div>
            ))}
          </div>
    ...
    return (
      <div style={{ padding: '24px', background: '#fef2f2', borderRadius: '12px', color: '#dc2626' }}>
